...
**"作为参照"技巧**：当报道涉及金额、估值、市场份额等数字时，主动搜索对比数据，帮助读者建立直观认知。

**使用场景：**
| 场景 | 示例 |
|------|------|
| 大额融资/估值 | "OpenAI估值8520亿美元，作为参照，这超过了Anthropic（约600亿美元）和Databricks（约430亿美元）的总和" |
| 收入/利润规模 | "月收入20亿美元，作为参照，这接近Anthropic年化收入（约30亿美元）的7成" |
| 用户/市场规模 | "日活1亿用户，相当于Twitter日活的1/3" |

**操作规范：**
1. **必须搜索核实**：使用WebSearch工具获取对比对象的当前真实数据（市值/估值/收入等）
2. **标注来源**：对比数据后标注来源和时间，如"（截至2026年4月）"
3. **选取有效参照**：选择读者熟悉的、量级相近的参照物，避免"相当于XX的0.01倍"这类无效对比
4. **无数据则放弃**：如果搜索不到可靠的对比数据，直接陈述原数字，不强行对比

**对比公式：**
```
原始数字 + "作为参照" + 对比对象当前数据 + 比例关系 + （数据来源/时间）
```

**对比后效果对比：**
- ❌ 原写法："OpenAI完成1220亿美元融资"（读者无感知）
- ❌ 错误对比："相当于Meta 2026年全年资本支出的90%"（融资额≠资本支出，维度不同）
- ✅ 正确对比："作为参照，这使其估值超过了Anthropic（约600亿美元）和Databricks（约430亿美元）的总和，但仍低于字节跳动（约2680亿美元）"（同维度：AI公司估值，同类竞品对比）
...
4. **混淆来源** - 每篇文章来源必须独立确认
5. **编造对比** - 写"相当于XX的YY倍/接近XX"前必须搜索核实XX的真实数据（市值/估值/收入），禁止凭记忆推测
6. **混淆因果关系** - 时间相近≠因果关系，写"作为回应/同时"前必须确认两件事的真实关联和时间线
...
| **因果关系** | 搜索"同时/作为回应/随后/开源社区回应" | 确认两件事是否真有因果关系，时间线是否准确 |
| **数据对比** | 搜索"相当于/接近/超过/是XX的N倍" | 必须搜索核实对比对象（XX）的当前真实数据 |
...
- ❌ 融资金额编造 → **改为"具体金额未披露"**
- ❌ 因果关系错误（两件事无因果却写成因果） → **删除因果表述，分开描述或修正关联**
- ❌ 数据对比错误（对比对象数据不准确） → **搜索核实真实数据后修正，或删除对比**
