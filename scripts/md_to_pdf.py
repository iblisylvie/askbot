...
def md_to_html(md_text, title="横纵分析报告", subtitle="横纵分析法深度研究报告",
    ...
    # 替换 CSS 中的页眉占位符
    css = CSS_TEMPLATE.replace("HEADER_TEXT", f"{title}  |  {subtitle}")

    # 构建封面
    author_html = f'<div class="meta">作者: {author}</div>' if author else ""
    cover_html = f"""
    <div class="cover">
...
def main():
    ...
    if not args.title or not args.subtitle:
        parser.error("--title 和 --subtitle 必须同时指定")
    ...
    base_url = os.path.dirname(os.path.abspath(args.input)) + os.sep
